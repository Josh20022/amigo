...
@section('content')
<section class="band_hero" style="background: url('{{ asset('storage/img/plaatjes/'.$offerCategory->plaatje->url) }}');">
    <div class="container">
        <h2 class="text-center">{{ $offerCategory->titel }}</h2>
    </div>
</section>
<section>
    <div class="page-info">
    ...
    <div class="container m-auto">
        <div class="mt-10 mb-20 grid gap-10 md:grid-cols-2 lg:gap-10 xl:grid-cols-3">
            @foreach ($acts as $act)
                <a href="{{ route('actdetail', ['offer' => $offer->id, 'offerCategory' => $act->offerCategory->id, 'act' => $act->id]) }}">
                    <div class="group cursor-pointer my-6 hover:my-6">
                        <div class="grid-img">
                            <div class="grid-img-container"
                                style="background-image: url('{{ asset('storage/img/plaatjes/'.$act->plaatje1->url) }}');  background-size: cover;">
                            </div>
                            <div class="grid-text">
                                <h2 class="img-text">
                                    {{ $act->titel }}
    ...
    {{-- <div class="goback">
        <a href="{{ route('offerCategories.index', $offer->id) }}" class="btn btn-warning">Ga terug naar categorieën</a>
    </div> --}}
</section>
@endsection